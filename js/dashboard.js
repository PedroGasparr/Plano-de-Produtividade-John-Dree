...
function renderOperationCard(operation) {
    ...
    const operationType = operation.type === 'simulation' ? 'Simulação' : 'Shipment';
    ...
    card.innerHTML = `
        ...
        <div class="operation-card-details">
            ${operation.type === 'simulation' ? 
                `<div>Endereço: ${operation.address}</div>` : 
                `<div>Shipment: ${operation.shipmentNumber}</div>
...
function confirmStartShipment() {
    ...
    if (!operatorName || !shipmentNumber || !surfNumber) {
    ...
    db.ref('operations').push(newOperation)
        .then(() => {
            showFeedback('Shipment iniciado com sucesso!', 'success', 'shipmentFeedback');
            setTimeout(() => {
                elements.shipmentModal.style.display = 'none';
                resetShipmentForm();
            }, 1500);
        ...
        .catch(error => {
            console.error('Erro ao iniciar shipment:', error);
            showFeedback('Erro ao iniciar shipment: ' + error.message, 'error', 'shipmentFeedback');
        })
        .finally(() => {
            elements.confirmShipmentBtn.disabled = false;
        });
...
function saveOperationHistory(operationId) {
    ...
    const historyData = {
        operationId,
        type: operation.type,
        ...(operation.type === 'simulation' ? { address: operation.address } : 
            { shipmentNumber: operation.shipmentNumber, surfNumber: operation.surfNumber }),
